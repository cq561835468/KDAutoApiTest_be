...
class Test_demo:
    def __init__(self):
        '''外部数据文件名，包头'''
        with open(os.path.dirname(os.path.realpath(__file__))+"\[DATA]33333333.json", 'r') as load_f:
    ...
    def Request(self):
        for x in self.load_dict:
            ...
            #----------------------------
            self.Response_Api = WebSocket_Request(name="VIID-POST").run(x["method"],x["url"],x["port"],x["path"],self.head,json.dumps(eval(json.dumps(body))))
